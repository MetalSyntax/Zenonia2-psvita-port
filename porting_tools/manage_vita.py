...
def download_latest_debug_files():
    ...
    if descargar_dmp in ['s', 'y', 'si', 'yes']:
        ...
        try:
            ...
            if latest_dmp:
                ...
                local_dmp_name = os.path.join(logs_folder, f"Zenonia2-{latest_dmp}")
    ...
    print("[*] Buscando el último log de Zenonia 2 en ux0:/data/zenonia-2/logs...")
    latest_log = None
    latest_log_time = 0
    try:
        ...
        if latest_log:
            ...
            print("[-] No se encontraron archivos de registro (.txt) en ux0:/data/zenonia-2/logs.")
    except all_errors as e:
...
def download_glsl_shaders():
    ...
    vita_glsl_dir = "ux0:/data/zenonia-2/glsl"
    project_root = os.path.dirname(BASE_DIR)
...
VITA_GAME_DATA_DIR = "/ux0:/data/zenonia-2/data"
LOCAL_DATA_REFERENCE_DIR = "apk_extract/assets"
...
def verify_data_assets():
    ...
    el volcado local de referencia (apk_extract/assets)
    contra lo que realmente está subido en ux0:data/zenonia-2/data/ en
...
def main():
    while True:
        ...
        print("2. Descargar el último dump (.dmp) y log (.txt) de Zenonia 2")
