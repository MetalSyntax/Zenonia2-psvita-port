...
extern int kuKernelCpuUnrestrictedMemcpy(void *dst, const void *src, SceSize len);
...
unsigned int sceLibcHeapSize = 4 * 1024 * 1024; // 4 MB for SCE Libc (system libs)
...
typedef struct { int type, p1, p2; } input_event;
...
static void queue_input_event(void *env, int type, int p1, int p2) {
    ...
    if (setInputEvent) setInputEvent(env, NULL, type, p1, p2);
...
static void apply_so_patches(so_module *mod) {
    ...
    if (cur != ble) {
        ...
        game_log("[PATCH] AVISO: bytes inesperados en 0xaec38 (0x%04x, se esperaba 0x%04x) -- parche omitido\n", cur, ble);
        return;
    }
    kuKernelCpuUnrestrictedMemcpy((void *)(mod->text_base + 0xaec38), &beq, sizeof(beq));
...
// Logs whatever sceDisplayGetFrameBuf currently reports as the buffer being scanned out,
// so we can tell from the log alone (without relying on what's visible on the TV/screen)
// whether vitaGL's swaps are actually taking over the display from debugScreen's own buffer.
...
void gl_init() {
    ...
    // vglInitExtended's return value is GL_TRUE only if the requested resolution had to be
    // reduced to fit the display's max (res_fallback in vitaGL's own source) -- it is NOT a
    // success/failure code, so GL_FALSE here (960x544 is the Vita's native resolution, never
    // falls back) is the expected, healthy result. Do not treat it as an init failure.
...
int main() {
	...
	if (res < 0) {
		...
		NativeResumeClet = (void *)so_symbol(&zenonia2_mod, "Java_com_gamevil_nexus2_Natives_NativeResumeClet");
		handleCletEvent = (void *)so_symbol(&zenonia2_mod, "Java_com_gamevil_nexus2_Natives_handleCletEvent");

		game_log("Symbols: JNI_OnLoad=%p NativeInit=%p NativeRender=%p NativeResize=%p setInputEvent=%p NativeResumeClet=%p handleCletEvent=%p\n",
			(void*)Game_JNI_OnLoad, (void*)NativeInit, (void*)NativeRender, (void*)NativeResize, (void*)setInputEvent, (void*)NativeResumeClet, (void*)handleCletEvent);
